...
const PREFIX       = 'manmin-indoor-hydrant-';
...
const PRECACHE_URLS = [
  ...
  /* v5.0 — html2canvas(JPG 저장)와 Google Fonts는 교차출처라 선캐싱 대신
     아래 fetch 핸들러의 Network-First 가 첫 온라인 로드 시 캐시한다. */
];
...
self.addEventListener('install', (event) => {
  ...
  event.waitUntil(
    caches.open(STATIC_CACHE)
      .then((cache) => {
        return Promise.allSettled(PRECACHE_URLS.map((u) => cache.add(mmReq(u)).catch((e) => console.warn('[SW] precache skip:', u, e)))).catch((e) => {
