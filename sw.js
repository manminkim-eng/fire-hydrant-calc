/* ═══════════════════════════════════════════════════════════════
   옥내소화전 펌프 계산서 — Service Worker
   ENGINEER KIM MANMIN · Ver 3.0
   전략:
     · 정적 자산  → Cache-First
     · HTML 페이지 → Network-First (최신 버전 우선 + 오프라인 폴백)
     · CDN/폰트   → Stale-While-Revalidate
     · 구버전 캐시 자동 삭제
   ═══════════════════════════════════════════════════════════════ */

const APP_ID       = 'manmin-hydrant';
...
<title>오프라인 — 옥내소화전 계산서</title>
<style>
*{box-sizing:border-box;margin:0;padding:0;}
body{font-family:'Noto Sans KR',sans-serif;background:#1e3a5f;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px;}
.card{background:white;border-radius:20px;padding:40px 36px;max-width:420px;width:100%;text-align:center;box-shadow:0 24px 64px rgba(0,0,0,0.3);}
...
.badge{display:inline-block;background:#fef2f2;color:#b91c1c;border:1px solid #fecaca;border-radius:20px;padding:6px 16px;font-size:12px;font-weight:700;margin-bottom:24px;}
button{background:#DC2626;color:white;border:none;border-radius:10px;padding:12px 28px;font-size:14px;font-weight:700;cursor:pointer;width:100%;}
button:active{background:#b91c1c;}
.footer{margin-top:20px;font-size:10px;color:#9ca3af;font-family:'Courier New',monospace;letter-spacing:0.08em;}
</style>
</head>
<body>
<div class="card">
...
self.addEventListener('message', e => {
  ...
  if (e.data?.type === 'CLEAR_CACHE') {
    caches.keys().then(keys =>
      Promise.all(keys.filter(k => k.startsWith(APP_ID)).map(k => caches.delete(k)))
